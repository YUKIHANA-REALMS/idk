...
class ResetPasswordFormType extends AbstractType
{
    ...
    public function buildForm(FormBuilderInterface $builder, array $options): void
    {
        $builder
            ->add('newPassword', PasswordType::class, [
                'label' => $this->translator->trans('indium.recovery.new_password'),
                'constraints' => [
                    new NotBlank([
                        'message' => $this->translator->trans('indium.recovery.enter_new_password'),
                    ]),
                    new Length([
                        'min' => 6,
                        'minMessage' => $this->translator->trans('indium.recovery.at_least_limit_characters'),
                        'max' => 4096,
                    ]),
                ],
            ...
            ->add('confirmPassword', PasswordType::class, [
                'label' => $this->translator->trans('indium.recovery.confirm_new_password'),
                'constraints' => [
                    new NotBlank([
                        'message' => $this->translator->trans('indium.recovery.please_confirm_password'),
                    ]),
    ...
    public function validatePasswordMatch(mixed $value, ExecutionContextInterface $context): void
    {
        ...
        if ($newPassword !== $value) {
            $context
                ->buildViolation($this->translator->trans('indium.recovery.passwords_must_match'))
                ->addViolation();
        }
    }
