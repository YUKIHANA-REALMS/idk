...
class CreateServerService extends AbstractActionServerService
{
    ...
    ): array
    {
        ...
        if ($aboutToBeCreatedEvent->isPropagationStopped()) {
            throw new Exception($this->translator->trans('indium.store.server_creation_blocked'));
        }
        ...
        try {
            ...
            $emailError = 'indium.email.creation_not_sent_misconfigured';
    ...
    ): PterodactylServer
    {
        try {
            ...
            throw new Exception(
                $this->translator->trans('indium.store.server_creation_failed') . ': ' . $exception->getMessage()
            );
    ...
    ): Server
    {
        ...
        if (empty($selectedPrice)) {
            throw new Exception($this->translator->trans('indium.store.price_not_found'));
        }
