...
readonly class UpdateServerService
{
    ...
    ): UpdateServerActionResult
    {
        try {
            ...
            $updateServerActionResult->addMessage(
                $this->translator->trans('indium.crud.server.build_updated_successfully'),
                CrudFlashMessageTypeEnum::SUCCESS,
            );
        } catch (Exception $exception) {
            $errorMessage = sprintf(
                '%s %s',
                $this->translator->trans('indium.crud.server.build_update_error'),
    ...
    ): UpdateServerActionResult
    {
        try {
            ...
            $updateServerActionResult->addMessage(
                $this->translator->trans('indium.crud.server.startup_updated_successfully'),
                CrudFlashMessageTypeEnum::SUCCESS,
            );
        } catch (Exception $exception) {
            $errorMessage = sprintf(
                '%s %s',
                $this->translator->trans('indium.crud.server.startup_update_error'),
                $exception->getMessage(),
            );
            $updateServerActionResult->addMessage($errorMessage, CrudFlashMessageTypeEnum::DANGER);
        } finally {
        ...
        if ($entityInstance->getIsSuspended() !== $pterodactylServer->get('suspended')) {
            if ($entityInstance->getIsSuspended()) {
                try {
                    ...
                    $updateServerActionResult->addMessage(
                        $this->translator->trans('indium.crud.server.suspended_successfully'),
                        CrudFlashMessageTypeEnum::SUCCESS,
                    );
                } catch (Exception $exception) {
                    $errorMessage = sprintf(
                        '%s %s',
                        $this->translator->trans('indium.crud.server.suspended_error'),
                        $exception->getMessage(),
                    );
                    $updateServerActionResult->addMessage($errorMessage, CrudFlashMessageTypeEnum::DANGER);
                }
                ...
                try {
                    ...
                    $updateServerActionResult->addMessage(
                        $this->translator->trans('indium.crud.server.unsuspended_successfully'),
                        CrudFlashMessageTypeEnum::SUCCESS,
                    );
                } catch (Exception $exception) {
                    $errorMessage = sprintf(
                        '%s %s',
                        $this->translator->trans('indium.crud.server.unsuspended_error'),
                        $exception->getMessage(),
                    );
                    $updateServerActionResult->addMessage($errorMessage, CrudFlashMessageTypeEnum::DANGER);
                }
    ...
    ): UpdateServerActionResult
    {
        ...
        if ($userChanged || $nameChanged) {
            try {
                ...
                $updateServerActionResult->addMessage(
                    $this->translator->trans('indium.crud.server.details_updated_successfully'),
                    CrudFlashMessageTypeEnum::SUCCESS,
                );
            } catch (Exception $exception) {
                $errorMessage = sprintf(
                    '%s %s',
                    $this->translator->trans('indium.crud.server.details_update_error'),
                    $exception->getMessage(),
                );
                $updateServerActionResult->addMessage($errorMessage, CrudFlashMessageTypeEnum::DANGER);
            }
