...
readonly class ServerUserService
{
        ...
        try {
            ...
            foreach ($currentSubusers['data'] ?? [] as $subuser) {
                if (isset($subuser['attributes']['email']) && $subuser['attributes']['email'] === $email) {
                    throw new Exception($this->translator->trans('indium.api.server_user.user_already_added', ['email' => $email]));
                }
            }
    ...
    ): array
    {
        ...
        $this->validateSubuserModification($server, $user, $email, $this->translator->trans('indium.api.server_user.modify_permissions'));
    ...
    ): void
    {
        ...
        $this->validateSubuserModification($server, $user, $email, $this->translator->trans('indium.api.server_user.delete_yourself_from_server'));
    ...
    private function validateSubuserModification(Server $server, UserInterface $user, string $targetEmail, string $action): void
    {
        ...
        if (!$isServerOwner && !$isAdmin && $user->getEmail() === $targetEmail) {
            throw new Exception($this->translator->trans('indium.api.server_user.cannot_modify_yourself', ['action' => $action]));
        }
    }
}
