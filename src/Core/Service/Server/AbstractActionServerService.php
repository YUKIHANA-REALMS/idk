...
abstract class AbstractActionServerService
{
    ...
    ): void
    {
        ...
        if (empty($price)) {
            throw new InvalidArgumentException($this->translator->trans('indium.store.price_not_found'));
        }
        ...
        if ($balancePaymentAmount > $user->getBalance()) {
            throw new InvalidArgumentException($this->translator->trans('indium.store.not_enough_funds'));
        }
