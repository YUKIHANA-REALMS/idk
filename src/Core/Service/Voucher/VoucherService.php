...
readonly class VoucherService
{
    ...
    public function redeemVoucher(string $code, ?float $orderAmount, UserInterface $user): RedeemVoucherActionResult
    {
        ...
        try {
            ...
            if ($requestedEvent->isPropagationStopped()) {
                $reason = $requestedEvent->getRejectionReason() ?? $this->translator->trans('indium.api.voucher.redemption_blocked');
            ...
            $successMessage = match ($voucher->getType()) {
                VoucherTypeEnum::BALANCE_TOPUP => $this->translator->trans('indium.api.voucher.successfully_applied_balance_topup', [
                    '%amount%' => number_format((float)$voucher->getValue(), 2),
                    '%currency%' => $currency,
                    '%newBalance%' => number_format($newBalance, 2),
                ]),
                VoucherTypeEnum::SERVER_DISCOUNT => $this->translator->trans('indium.api.voucher.successfully_applied_server_discount', [
                    '%value%' => $voucher->getValue(),
                ]),
                VoucherTypeEnum::PAYMENT_DISCOUNT => $this->translator->trans('indium.api.voucher.successfully_applied_payment_discount', [
                    '%value%' => $voucher->getValue(),
                ]),
            };
    ...
    public function getValidVoucher(string $code): Voucher
    {
        ...
        if (empty($voucher) || !empty($voucher->getDeletedAt())) {
            throw new Exception($this->translator->trans('indium.api.voucher.not_found'));
        }

        if (!empty($voucher->getExpirationDate()) && $voucher->getExpirationDate() < new DateTime()) {
            throw new Exception($this->translator->trans('indium.api.voucher.expired'));
        }

        if (!empty($voucher->getMaxGlobalUses()) && $voucher->getUsedCount() >= $voucher->getMaxGlobalUses()) {
            throw new Exception($this->translator->trans('indium.api.voucher.max_global_uses_reached'));
        }
    ...
    private function validateNewAccountRequirementIfNeeded(Voucher $voucher, UserInterface $user): void
    {
        ...
        if (
            ...
            throw new Exception($this->translator->trans('indium.api.voucher.only_for_new_accounts'));
        }
    }
    ...
    private function validateOneUsePerUserRequirementIfNeeded(Voucher $voucher, UserInterface $user): void
    {
        ...
        if ($this->voucherUsageRepository->hasUsedVoucher($voucher->getCode(), $user->getId())) {
            throw new Exception($this->translator->trans('indium.api.voucher.already_used'));
        }
    }
    ...
    private function validateMinimumTopupAmountRequirementIfNeeded(Voucher $voucher, UserInterface $user): void
    {
        ...
        if ($voucher->getMinimumTopupAmount() > $userPaymentsSum) {
            $exceptionMessage = $this->translator->trans('indium.api.voucher.minimum_top_up_amount_required', [
    ...
    private function validateMinimumOrderAmountRequirementIfNeeded(?float $orderAmount, Voucher $voucher): void
    {
        ...
        if ($voucher->getMinimumOrderAmount() > $orderAmount) {
            $exceptionMessage = $this->translator->trans('indium.api.voucher.minimum_order_amount_required', [
