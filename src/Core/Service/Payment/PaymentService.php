...
readonly class PaymentService
{
    ...
    ): string
    {
        ...
        if (!$paymentProvider->isConfigured()) {
            throw new Exception($this->translator->trans('indium.payment.gateway_not_configured'));
        }

        if (!in_array($currency, $paymentProvider->getSupportedCurrencies(), true)) {
            throw new Exception($this->translator->trans('indium.payment.currency_not_supported'));
        }
        ...
        if (empty($session)) {
            throw new Exception($this->translator->trans('indium.recharge.failed_to_create_payment'));
        }
    ...
    ): string
    {
        ...
        if (empty($payment)) {
            throw new Exception($this->translator->trans('indium.recharge.payment_not_found'));
        }
        ...
        if ($retrievedSession->getPaymentStatus() === PaymentStatusEnum::PAID->value) {
            throw new Exception($this->translator->trans('indium.recharge.payment_already_processed'));
        }

        $url = $retrievedSession->getUrl();
        if ($url === null) {
            throw new PaymentExpiredException($this->translator->trans('indium.recharge.payment_expired'));
        }
    ...
    public function finalizePayment(UserInterface $user, string $sessionId): ?string
    {
        ...
        $payment = $this->paymentRepository->findOneBy(['sessionId' => $sessionId]);
        if (empty($payment)) {
            return $this->translator->trans('indium.recharge.payment_not_found');
        }

        $paymentProvider = $this->gatewayManager->getProvider($payment->getGateway());
        if ($paymentProvider === null) {
            return $this->translator->trans('indium.payment.gateway_not_found');
        }
        ...
        if ($payment->getStatus() === $session->getPaymentStatus()) {
            return $this->translator->trans('indium.recharge.payment_already_processed');
        }
        ...
        if ($session->getPaymentStatus() === $paymentProvider::PAID_STATUS) {
            ...
            if ($aboutToBeAddedEvent->isPropagationStopped()) {
                return $aboutToBeAddedEvent->getRejectionReason() 
                    ?? $this->translator->trans('indium.recharge.payment_rejected_by_plugin');
            }
            ...
            $emailMessage = new SendEmailMessage(
                $user->getEmail(),
                $this->translator->trans('indium.email.payment.subject'),
