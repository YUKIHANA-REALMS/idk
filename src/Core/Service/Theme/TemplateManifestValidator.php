...
class TemplateManifestValidator
{
    ...
    public function validate(TemplateManifestDTO $manifest): array
    {
        ...
        $errors = array_merge($errors, $this->validateIndiumVersion($manifest->indiumVersion));
    ...
    private function validateIndiumVersion(string $version): array
    {
        if (empty($version)) {
            return ['Indium version is required'];
        }
        ...
        try {
            ...
            return ['Invalid Indium version format. Must be semantic version (e.g., 1.0.0)'];
        }
    }
