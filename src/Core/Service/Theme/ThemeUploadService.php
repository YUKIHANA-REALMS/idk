...
class ThemeUploadService
{
    ...
    public function __construct(
        ...
        private readonly string $currentIndiumVersion,
    ...
    private function checkVersionCompatibility(TemplateManifestDTO $manifest): array
    {
        ...
        // Check if theme targets older Indium Panel version
        if (version_compare($manifest->indiumVersion, $this->currentIndiumVersion, '<')) {
            $warnings[] = new ThemeUploadWarningDTO(
                type: 'outdated_indium_version',
                severity: 'warning',
                message: "Theme targets Indium Panel v{$manifest->indiumVersion}, current version is v{$this->currentIndiumVersion}. May not work correctly.",
                details: [
                    'theme_version' => $manifest->indiumVersion,
                    'current_version' => $this->currentIndiumVersion,
                ]
            );
        }
