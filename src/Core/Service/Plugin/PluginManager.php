...
readonly class PluginManager
{
    ...
    public function registerPlugin(string $pluginPath, PluginManifestDTO $manifest, ?string $zipHash = null): Plugin
    {
        ...
        // Validate Indium Panel compatibility
        if (!$this->manifestValidator->isCompatibleWithIndium($manifest)) {
    ...
    public function enablePlugin(Plugin $plugin): void
    {
        ...
        // Verifies indium.indicloud.xyz license before enabling plugin.
        // Removing this block allows unlicensed marketplace plugins to run,
        // harming their developers. Core is overwritten on each Indium Panel update.
        ...
        if ($implementsInterface) {
            ...
            if ($licenseResult->apiUnavailable) {
                throw new LicenseVerificationException(
                    'License verification failed: indium.indicloud.xyz is currently unavailable. Please try again later.'
                );
    ...
    private function createPluginEntityFromManifest(string $pluginPath, PluginManifestDTO $manifest): Plugin
    {
        ...
        $plugin->setIndiumMinVersion($manifest->getMinIndiumVersion());
        $plugin->setIndiumMaxVersion($manifest->getMaxIndiumVersion());
