...
class MenuBuilder
{
    ...
    {
        $mainItems = [
            MenuItem::section($this->translator->trans('indium.crud.menu.menu')),
            MenuItem::linkToDashboard($this->translator->trans('indium.crud.menu.dashboard'), 'fa fa-home'),
        ];

        // My Servers
        if ($this->security->isGranted(PermissionEnum::ACCESS_MY_SERVERS->value)) {
            $mainItems[] = MenuItem::linkToRoute(
                $this->translator->trans('indium.crud.menu.my_servers'),
                'fa fa-server',
                'servers'
            );
        }
        ...
        if ($this->security->isGranted(PermissionEnum::ACCESS_SHOP->value)) {
            $mainItems[] = MenuItem::linkToRoute(
                $this->translator->trans('indium.crud.menu.shop'),
                'fa fa-shopping-cart',
                'panel_store'
            );
        }
        ...
        if ($this->security->isGranted(PermissionEnum::ACCESS_WALLET->value)) {
            $mainItems[] = MenuItem::linkToRoute(
                $this->translator->trans('indium.crud.menu.wallet'),
                'fa fa-wallet',
                'recharge_balance'
            );
        }
        ...
        $mainItems[] = MenuItem::subMenu($this->translator->trans('indium.crud.menu.my_account'), 'fa fa-user')->setSubItems([
            MenuItem::linkToCrud($this->translator->trans('indium.crud.menu.payments'), 'fa fa-money', UserPayment::class),
            MenuItem::linkToCrud($this->translator->trans('indium.crud.menu.account_settings'), 'fa fa-user-cog', UserAccount::class),
        ]);
    ...
    {
        $adminItems = [
            MenuItem::section($this->translator->trans('indium.crud.menu.administration')),
        ];

        // Overview (always visible if user has ACCESS_ADMIN_OVERVIEW)
        $adminItems[] = MenuItem::linkToRoute(
            $this->translator->trans('indium.crud.menu.overview'),
            'fa fa-gauge',
            'admin_overview'
        );
        ...
        if ($this->security->isGranted(PermissionEnum::ACCESS_SERVERS->value)) {
            $adminItems[] = MenuItem::linkToCrud(
                $this->translator->trans('indium.crud.menu.servers'),
                'fa fa-server',
                Server::class
            );
        }
        ...
        if ($this->security->isGranted(PermissionEnum::ACCESS_PAYMENTS->value)) {
            $adminItems[] = MenuItem::linkToCrud(
                $this->translator->trans('indium.crud.menu.payments'),
                'fa fa-money',
                Payment::class
            );
        }
        ...
        if ($this->security->isGranted(PermissionEnum::ACCESS_USERS->value)) {
            $adminItems[] = MenuItem::linkToCrud(
                $this->translator->trans('indium.crud.menu.users'),
                'fa fa-user',
                User::class
            );
        }
    ...
    private function buildShopSubmenu(): ?SubMenuItem
    {
        ...
        if ($this->security->isGranted(PermissionEnum::ACCESS_CATEGORIES->value)) {
            $shopItems[] = MenuItem::linkToCrud(
                $this->translator->trans('indium.crud.menu.categories'),
                'fa fa-list',
                Category::class
            );
        }
        ...
        if ($this->security->isGranted(PermissionEnum::ACCESS_PRODUCTS->value)) {
            $shopItems[] = MenuItem::linkToCrud(
                $this->translator->trans('indium.crud.menu.products'),
                'fa fa-sliders-h',
                Product::class
            );
        }
    ...
    private function buildLogsSubmenu(): ?SubMenuItem
    {
        ...
        if ($this->security->isGranted(PermissionEnum::ACCESS_SYSTEM_LOGS->value)) {
            $logItems[] = MenuItem::linkToCrud(
                $this->translator->trans('indium.crud.menu.logs'),
        ...
        if ($this->security->isGranted(PermissionEnum::ACCESS_EMAIL_LOGS->value)) {
            $logItems[] = MenuItem::linkToCrud(
                $this->translator->trans('indium.crud.menu.email_logs'),
                'fa fa-envelope',
                EmailLog::class
            );
        }
        ...
        if ($this->security->isGranted(PermissionEnum::ACCESS_SERVER_LOGS->value)) {
            $logItems[] = MenuItem::linkToCrud(
                $this->translator->trans('indium.crud.menu.server_logs'),
                'fa fa-bars',
                ServerLog::class
            );
        }
    ...
    private function buildSettingsSubmenu(callable $settingsUrlGenerator): ?SubMenuItem
    {
        ...
        if ($this->security->isGranted(PermissionEnum::ACCESS_SETTINGS_GENERAL->value)) {
            $settingsItems[] = MenuItem::linkToUrl(
                $this->translator->trans('indium.crud.menu.general'),
                'fa fa-cog',
        ...
        if ($this->security->isGranted(PermissionEnum::ACCESS_SETTINGS_PTERODACTYL->value)) {
            $settingsItems[] = MenuItem::linkToUrl(
                $this->translator->trans('indium.crud.menu.pterodactyl'),
                'fa fa-network-wired',
        ...
        if ($this->security->isGranted(PermissionEnum::ACCESS_SETTINGS_SECURITY->value)) {
            $settingsItems[] = MenuItem::linkToUrl(
                $this->translator->trans('indium.crud.menu.security'),
                'fa fa-shield-halved',
        ...
        if ($this->security->isGranted(PermissionEnum::ACCESS_SETTINGS_PAYMENT->value)) {
            $settingsItems[] = MenuItem::linkToUrl(
                $this->translator->trans('indium.crud.menu.payment_gateways'),
        ...
        if ($this->security->isGranted(PermissionEnum::ACCESS_SETTINGS_EMAIL->value)) {
            $settingsItems[] = MenuItem::linkToUrl(
                $this->translator->trans('indium.crud.menu.email'),
                'fa fa-envelope',
        ...
        if ($this->security->isGranted(PermissionEnum::ACCESS_SETTINGS_THEME->value)) {
            $settingsItems[] = MenuItem::linkToUrl(
                $this->translator->trans('indium.crud.menu.appearance'),
                'fa fa-brush',
        ...
        return MenuItem::subMenu(
            $this->translator->trans('indium.crud.menu.settings'),
            'fa fa-cogs'
    ...
    private function buildThemesSubmenu(): ?SubMenuItem
    {
        ...
        $themeItems[] = MenuItem::linkToUrl(
            $this->translator->trans('indium.crud.menu.manage_themes'),
            'fa fa-brush',
            $this->adminUrlGenerator
        ...
        if ($this->security->isGranted(PermissionEnum::UPLOAD_THEME->value)) {
            $themeItems[] = MenuItem::linkToUrl(
                $this->translator->trans('indium.theme.upload.upload_theme'),
                'fa fa-upload',
                $this->adminUrlGenerator
        ...
        return MenuItem::subMenu(
            $this->translator->trans('indium.crud.menu.themes'),
            'fa fa-brush'
    ...
    private function buildPluginsSubmenu(): ?SubMenuItem
    {
        ...
        $pluginItems[] = MenuItem::linkToCrud(
            $this->translator->trans('indium.crud.plugin.manage_plugins'),
            'fa fa-list',
            Plugin::class
        );
        ...
        if ($this->security->isGranted(PermissionEnum::UPLOAD_PLUGIN->value)) {
            $pluginItems[] = MenuItem::linkToUrl(
                $this->translator->trans('indium.plugin.upload.upload_plugin'),
                'fa fa-upload',
                $this->adminUrlGenerator
        ...
        return MenuItem::subMenu(
            $this->translator->trans('indium.crud.plugin.plugins'),
            'fa fa-puzzle-piece'
    ...
    private function buildVouchersSubmenu(): ?SubMenuItem
    {
        ...
        if ($this->security->isGranted(PermissionEnum::ACCESS_VOUCHERS->value)) {
            $voucherItems[] = MenuItem::linkToCrud(
                $this->translator->trans('indium.crud.menu.vouchers'),
                'fa fa-gift',
                Voucher::class
            );
        }
        ...
        if ($this->security->isGranted(PermissionEnum::ACCESS_VOUCHER_USAGES->value)) {
            $voucherItems[] = MenuItem::linkToCrud(
                $this->translator->trans('indium.crud.menu.voucher_usages'),
                'fa fa-list',
                VoucherUsage::class
            );
        }
    ...
    private function buildRolesPermissionsSubmenu(): ?SubMenuItem
    {
        ...
        if ($this->security->isGranted(PermissionEnum::ACCESS_ROLES->value)) {
            $rolesPermissionsItems[] = MenuItem::linkToCrud(
                $this->translator->trans('indium.crud.menu.roles'),
                'fa fa-user-tag',
                Role::class
            );
        }
        ...
        if ($this->security->isGranted(PermissionEnum::ACCESS_PERMISSIONS->value)) {
            $rolesPermissionsItems[] = MenuItem::linkToCrud(
                $this->translator->trans('indium.crud.menu.permissions'),
                'fa fa-lock',
                Permission::class
            );
        }
        ...
        return MenuItem::subMenu(
            $this->translator->trans('indium.crud.menu.roles_and_permissions'),
            'fa fa-user-shield'
