...
readonly class EmailContextBuilderService
{
    ...
    private function buildFormattedDescription(
        ...
        return match ($pricingType) {
            ProductPriceTypeEnum::SLOT->value => $this->translator->trans('indium.email.pricing.slot_format', [
                '{{ slots }}' => $slots ?? 1,
                '{{ price }}' => $this->priceFormatterService->formatPrice($basePrice),
                '{{ currency }}' => $currency,
                '{{ total }}' => $this->priceFormatterService->formatPrice($finalPrice),
            ]),
            ProductPriceTypeEnum::STATIC->value => $this->translator->trans('indium.email.pricing.static_format', [
                '{{ price }}' => $this->priceFormatterService->formatPrice($finalPrice),
                '{{ currency }}' => $currency,
            ]),
            ProductPriceTypeEnum::ON_DEMAND->value => $this->translator->trans('indium.email.pricing.on_demand_format', [
                '{{ price }}' => $this->priceFormatterService->formatPrice($finalPrice),
                '{{ currency }}' => $currency,
            ]),
            default => $this->translator->trans('indium.email.pricing.static_format', [
                '{{ price }}' => $this->priceFormatterService->formatPrice($finalPrice),
                '{{ currency }}' => $currency,
            ]),
        };
