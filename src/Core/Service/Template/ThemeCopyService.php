...
class ThemeCopyService
{
    ...
    public function validateThemeName(string $themeName): array
    {
        ...
        if (empty($themeName)) {
            $errors[] = $this->translator->trans('indium.crud.theme.validation.name_required');
            return $errors;
        }

        if (strlen($themeName) < 3) {
            $errors[] = $this->translator->trans('indium.crud.theme.validation.name_too_short');
        }

        if (strlen($themeName) > 50) {
            $errors[] = $this->translator->trans('indium.crud.theme.validation.name_too_long');
        }

        if (!preg_match('/^[a-z0-9\-_]+$/', $themeName)) {
            $errors[] = $this->translator->trans('indium.crud.theme.validation.name_invalid_chars');
        }

        if (preg_match('/^[\-_]|[\-_]$/', $themeName)) {
            $errors[] = $this->translator->trans('indium.crud.theme.validation.name_invalid_edges');
        }

        $reserved = [TemplateService::DEFAULT_THEME, 'system', 'admin', 'test'];
        if (in_array($themeName, $reserved, true)) {
            $errors[] = $this->translator->trans('indium.crud.theme.validation.name_reserved');
        }

        $themePath = $this->projectDir . '/themes/' . $themeName;
        if ($this->filesystem->exists($themePath)) {
            $errors[] = $this->translator->trans('indium.crud.theme.validation.name_already_exists');
        }
