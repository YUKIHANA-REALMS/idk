...
class TemplateService
{
    ...
    public function getTemplateInfo(string $templateName): array
    {
        ...
        $currentIndiumVersion = $this->systemVersionService->getCurrentVersion();
        $indiumVersionIndex = $this->translator->trans('indium.crud.setting.template.indiumVersion');
        $templateIndiumVersion = $templateInfo[$indiumVersionIndex] ?? null;
        $isOutdated = !empty($templateIndiumVersion)
            && version_compare($templateIndiumVersion, $currentIndiumVersion, '<');

        if ($isOutdated) {
            $templateInfo[$indiumVersionIndex] = sprintf(
                '%s (%s %s)',
                $templateIndiumVersion,
                '<i class="fas fa-exclamation-triangle text-warning"></i>',
                $this->translator->trans('indium.crud.setting.template.outdated')
            );
        }
        ...
        if (!empty($translationErrors)) {
            $translationsLabel = $this->translator->trans('indium.crud.setting.template.translations');
            $templateInfo[$translationsLabel] = sprintf(
                ...
                $this->translator->trans('indium.crud.setting.template.translation_validation_errors'),
    ...
    public function getAllThemesWithActiveContexts(
        ...
        foreach ($allThemes as $themeName) {
            ...
            $themes[] = new ThemeDTO(
                ...
                indiumVersion: $metadata['indiumVersion'] ?? 'unknown',
    ...
    private function prepareTemplateMetadata(array $templateMetadata, bool $loadRawData = false): array
    {
        ...
        foreach ($templateMetadata as $key => $value) {
            $label = !$loadRawData
                ? $this->translator->trans(sprintf('indium.crud.setting.template.%s', $key))
                : $key;
