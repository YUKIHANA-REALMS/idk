...
class ServerProduct extends AbstractEntity implements ProductInterface
{
    ...
    public function validatePrices(ExecutionContextInterface $context): void
    {
        if (count($this->getPrices()) === 0) {
            $context->buildViolation('indium.crud.product.at_least_one_price_required')
        ...
        if (empty($this->getSelectedPrice())) {
            $context->buildViolation('indium.crud.product.at_least_one_selected_price_required')
                ->setTranslationDomain('messages')
                ->atPath('prices')
                ->addViolation();
        }
        ...
        if (count($selectedPrices) > 1) {
            $context->buildViolation('indium.crud.product.only_one_selected_price_allowed')
                ->setTranslationDomain('messages')
                ->atPath('prices')
                ->addViolation();
        }
