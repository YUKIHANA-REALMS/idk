...
class SyncServersHandler implements HandlerInterface
{
    ...
    public function handle(bool $dryRun = false, bool $auto = false): void
    {
        ...
        $context = $this->eventContextService->buildCliContext('indium:sync-servers', [
            'limit' => $this->limit,
            'dryRun' => $dryRun,
            'auto' => $auto,
        ]);
        ...
        try {
            $this->io->title('Indium Panel Server Synchronization');
            ...
            $this->io->section('Cleaning up orphaned servers in Indium Panel...');
            ...
            $this->io->success(sprintf(
                'Server synchronization completed. Found %d existing servers in Pterodactyl, %s %d orphaned servers in Indium Panel. (Skipped: %d, Failed: %d)',
                $stats['pterodactylServersFound'],
