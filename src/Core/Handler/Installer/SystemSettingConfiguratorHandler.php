...
class SystemSettingConfiguratorHandler
{
    ...
    private function askForConfigureUser(SymfonyStyle $io): void
    {
        if ($io->ask('Do you want to create user?', 'yes') === 'yes') {
            ...
            $io->text('Please provide details for a new administrator account that will be created in both Indium Panel and Pterodactyl. You cannot use an existing account.');
    ...
    private function askForTelemetryConsent(SymfonyStyle $io): void
    {
        ...
        $io->text('Help improve Indium Panel by sending anonymous usage data.');
        $io->text('We collect anonymous telemetry to improve Indium Panel. Only installation events and errors are tracked.');
