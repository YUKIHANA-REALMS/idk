...
class MigrateServersHandler implements HandlerInterface
{
    ...
    public function handle(bool $dryRun = false): void
    {
        ...
        try {
            ...
            $indiumServers = $this->getIndiumServers();
            $indiumUsers = $this->getIndiumUsers();
            ...
            foreach ($pterodactylServers as $pterodactylServer) {
                ...
                // Skip: already exists
                if ($this->isServerAlreadyExists($serverArray, $indiumServers)) {
                    ...
                    $this->io->info(sprintf(
                        'Server %s #%s already exists in Indium Panel, skipping...',
                        $serverArray['name'],
                        $serverArray['identifier'],
                    ));
                    continue;
                ...
                if (!in_array($pterodactylServerOwner->get('email'), array_column($indiumUsers, 'email'))) {
                    $stats['serversSkipped']++;
                    ...
                    $this->io->warning(sprintf(
                        'User %s does not exist in Indium Panel, skipping server %s #%s...',
                        $pterodactylServerOwner->get('email'),
                        $serverArray['name'],
                        $serverArray['identifier'],
                    ));
                ...
                $this->io->info('You need to set the base price and duration for the server in Indium Panel.');
    ...
    {
        $indiumUser = $this->userRepository->findOneBy(['email' => $pterodactylServerOwnerEmail]);
        if (empty($indiumUser)) {
        ...
        $serverEntity = $this->migrateServerEntity($indiumUser, $pterodactylServer, $duration);
    ...
    private function isServerAlreadyExists(array $pterodactylServer, array $indiumServers): bool
    {
        return in_array(
            $pterodactylServer['identifier'],
            array_column($indiumServers, 'pterodactylServerIdentifier'),
        );
    }
    ...
    private function getIndiumServers(): array
    {
        return array_map(
            fn(Server $server) => [
    ...
    private function getIndiumUsers(): array
    {
        return array_map(
