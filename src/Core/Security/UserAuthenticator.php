...
class UserAuthenticator extends AbstractLoginFormAuthenticator
{
    ...
    public function authenticate(Request $request): Passport
    {
        ...
        if ($this->captchaService->isCaptchaEnabled()
            ...
            throw new CustomUserMessageAuthenticationException($this->translator->trans('indium.login.invalid_captcha'));
        }

        $user = $this->userRepository->findByEmailIncludingDeleted($email);
        if (!empty($user)) {
            if ($user->isDeleted()) {
                throw new CustomUserMessageAuthenticationException($this->translator->trans('indium.login.invalid_credentials'));
            }
            if ($user->isBlocked()) {
                throw new CustomUserMessageAuthenticationException($this->translator->trans('indium.login.user_blocked'));
            }
        }
