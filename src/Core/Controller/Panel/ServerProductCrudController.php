...
class ServerProductCrudController extends AbstractPanelController
{
    ...
    public function configureFields(string $pageName): iterable
    {
        ...
        $this->fields = [
            FormField::addTab($this->translator->trans('indium.crud.product.server_details'))
                ->setIcon('fa fa-info-circle'),

            Field::new('user', $this->translator->trans('indium.crud.server.user'))
                ...
                ->setRequired(true)
                ->setColumns(6)
                ->setHelp($this->translator->trans('indium.admin.server_create.select_user_help'))
                ->hideWhenUpdating(),

            ChoiceField::new('baseProduct', $this->translator->trans('indium.crud.server.base_product'))
                ->onlyOnForms()
                ->setChoices($productChoices)
                ->setRequired(false)
                ->setColumns(6)
                ->setHelp($this->translator->trans('indium.admin.server_create.base_product_help'))
                ->setFormTypeOption('mapped', false)
                ->hideWhenUpdating(),

            BooleanField::new('freeServer', $this->translator->trans('indium.admin.server_create.free_server'))
                ->onlyOnForms()
                ->setColumns(12)
                ->setHelp($this->translator->trans('indium.admin.server_create.free_server_help'))
                ->setFormTypeOption('mapped', false)
                ->hideWhenUpdating(),

            BooleanField::new('sendCreationEmail', $this->translator->trans('indium.admin.server_create.send_creation_email'))
                ->onlyOnForms()
                ->setColumns(12)
                ->setHelp($this->translator->trans('indium.admin.server_create.send_creation_email_help'))
            ...
            IdField::new('server.id')
                ->hideOnForm()
                ->setColumns(3),
            IntegerField::new('server.pterodactylServerId', $this->translator->trans('indium.crud.server.pterodactyl_server_id'))
                ->setDisabled()
                ->hideWhenCreating()
                ->setColumns(3),
            TextField::new('server.pterodactylServerIdentifier', $this->translator->trans('indium.crud.server.pterodactyl_server_identifier'))
                ->setDisabled()
                ->hideWhenCreating()
                ->setColumns(3),
            TextField::new('server.user', $this->translator->trans('indium.crud.server.user'))
                ->setDisabled()
                ->hideWhenCreating()
                ->setColumns(3),
            TextField::new($isNewPage ? 'newServerName' : 'server.name', $this->translator->trans('indium.crud.server.name'))
                ->setRequired(true)
                ->setColumns(12)
                ->setFormTypeOptions($isNewPage ? ['mapped' => false] : []),

            DateTimeField::new('server.createdAt', $this->translator->trans('indium.crud.server.created_at'))
                ->setDisabled()
                ->hideWhenCreating()
                ->setColumns(3),
            DateTimeField::new('server.deletedAt', $this->translator->trans('indium.crud.server.deleted_at'))
                ->setDisabled()
                ->hideWhenCreating()
                ->setColumns(3),

            DateTimeField::new($isNewPage ? 'newServerExpiresAt' : 'server.expiresAt', $this->translator->trans('indium.crud.server.expires_at'))
                ->setRequired(true)
                ->setColumns(3)
                ->setFormTypeOptions($isNewPage ? ['mapped' => false] : []),

            BooleanField::new($isNewPage ? 'newServerIsSuspended' : 'server.isSuspended', $this->translator->trans('indium.crud.server.is_suspended'))
                ->setColumns(3)
                ->setFormTypeOptions($isNewPage ? ['mapped' => false] : []),

            BooleanField::new($isNewPage ? 'newServerAutoRenewal' : 'server.autoRenewal', $this->translator->trans('indium.crud.server.auto_renewal'))
                ->setColumns(3)
                ->setFormTypeOptions($isNewPage ? ['mapped' => false] : [])
                ->hideOnIndex(),

            FormField::addTab($this->translator->trans('indium.crud.product.build_details'))
                ->setIcon('fa fa-asterisk'),
            TextField::new('name', $this->translator->trans('indium.crud.product.build_name'))
                ->setColumns(7),
            FormField::addRow(),
            AssociationField::new('originalProduct', $this->translator->trans('indium.crud.server_product.original_product'))
                ->setColumns(7)
                ->setDisabled()
                ->hideWhenCreating(),
            AssociationField::new('server', $this->translator->trans('indium.crud.product.server'))
                ->setColumns(5)
                ->setDisabled()
                ->hideWhenCreating(),
            ...
            FormField::addPanel(sprintf(
                ...
                $this->translator->trans('indium.crud.product.product_connections_note'),
            ))
            ...
            BooleanField::new('allowUserSelectLocation', $this->translator->trans('indium.crud.product.allow_user_select_location'))
                ...
                ->hideOnIndex()
                ->hideWhenCreating(),
            NumberField::new('selectedNodeId', $this->translator->trans('indium.crud.server_product.selected_node_id'))
                ->setDisabled()
                ->setColumns(6)
                ->hideOnIndex()
                ->hideWhenCreating(),
    ...
    public function configureCrud(Crud $crud): Crud
    {
        ...
        $crud
            ->setEntityLabelInSingular($this->translator->trans('indium.crud.product.server_build'))
            ->setEntityLabelInPlural($this->translator->trans('indium.crud.product.server_builds'))
    ...
    private function createNewServer(ServerProduct $serverProduct): void
    {
        try {
            ...
            $this->addFlash('success', $this->translator->trans(
                'indium.admin.server_create.success',
                ['%user%' => $user->getEmail()]
            ));
        } catch (Exception $e) {
    ...
    private function loadUserFromForm(array $formData): User
    {
        ...
        if (!$user) {
            throw new Exception($this->translator->trans('indium.error.user_not_found'));
        }
    ...
    private function getServerBuildFields(): array
    {
        $panelFieldLabel = sprintf(
            '<i class="fa fa-info-circle pe-2"></i> %s',
            $this->translator->trans('indium.crud.product.server_build_offline_alert')
        );
