...
class ThemeCrudController extends AbstractPanelController
{
    ...
    public function index(AdminContext $context): Response
    {
        ...
        $viewData = [
            ...
            'page_title' => $this->translator->trans('indium.crud.menu.manage_themes'),
        ];
    ...
    public function viewDetails(AdminContext $context): Response
    {
        ...
        if (!$this->templateService->themeSupportsContext($themeName, $themeContext)) {
            $this->addFlash('danger', sprintf(
                $this->translator->trans('indium.crud.theme.theme_not_found'),
                $themeName
            ));
    ...
    public function setDefaultTheme(AdminContext $context): RedirectResponse
    {
        ...
        try {
            ...
            if ($result !== null) {
                if ($result->fileBlacklisted) {
                    $this->addFlash('danger', $this->translator->trans(
                        'indium.license.theme_blacklisted',
                ...
                if ($result->apiUnavailable) {
                    $this->addFlash('info', $this->translator->trans('indium.license.api_unavailable_warning'));
            ...
            $this->addFlash('success', sprintf(
                $this->translator->trans('indium.crud.theme.set_as_default_success'),
                $displayName,
                $themeContext
            ));
        } catch (\Exception $e) {
    ...
    public function deleteTheme(AdminContext $context): RedirectResponse
    {
        ...
        $successMessage = $this->translator->trans('indium.crud.theme.delete_success');
        $errorMessage = $this->translator->trans('indium.crud.theme.delete_error');
        $preventedMessage = $this->translator->trans('indium.crud.theme.deletion_prevented');
    ...
    public function copyTheme(AdminContext $context): RedirectResponse
    {
        ...
        try {
            ...
            $this->addFlash('success', sprintf(
                $this->translator->trans('indium.crud.theme.copy_success'),
                $sourceThemeName,
                $newThemeName
            ));
        } catch (\InvalidArgumentException $e) {
            ...
            $this->addFlash('danger', sprintf(
                $this->translator->trans('indium.crud.theme.copy_error'),
                $e->getMessage()
            ));
        }
    ...
    public function exportTheme(AdminContext $context): Response
    {
        ...
        try {
            ...
            $this->addFlash('danger', sprintf(
                $this->translator->trans('indium.crud.theme.export_error'),
                $e->getMessage()
            ));
            return $this->redirect($contextRedirectUrl);
        }
    ...
    public function uploadTheme(AdminContext $context): Response
    {
        ...
        return $this->renderWithEvent(
            ...
            [
                ...
                'page_title' => $this->translator->trans('indium.theme.upload.title'),
                'back_url' => $backUrl,
    ...
    public function processUpload(): Response
    {
        ...
        if (!$form->isSubmitted() || !$form->isValid()) {
            $this->addFlash('danger', $this->translator->trans('indium.theme.upload.errors.invalid_form'));
        ...
        try {
            ...
            $this->addFlash('success', sprintf(
                $this->translator->trans('indium.theme.upload.success'),
            ...
            if ($result->hasWarnings()) {
                ...
                foreach ($groupedWarnings as $type => $data) {
                    $warningMessage = $this->translator->trans('indium.theme.upload.warning.' . $type);

                    if ($data['count'] > 1) {
                        $warningMessage .= sprintf(' (%d %s)',
                            $data['count'],
                            $this->translator->trans('indium.theme.upload.occurrences')
                        );
                    }
            ...
            $this->addFlash('danger', $this->translator->trans('indium.theme.upload.errors.invalid_structure'));
            ...
            $this->addFlash('danger', sprintf(
                $this->translator->trans('indium.theme.upload.errors.invalid_manifest'),
                $errors
            ));
            ...
            $this->addFlash('danger', $this->translator->trans('indium.theme.upload.errors.security_critical'));
        } catch (\Exception $e) {
            $this->dispatchDataEvent(
                ThemeUploadFailedEvent::class,
                $request,
            ...
            $this->addFlash('danger', sprintf(
                $this->translator->trans('indium.theme.upload.errors.generic'),
                $e->getMessage()
            ));
        }
    ...
    private function getThemeActions(ThemeDTO $theme): array
    {
        ...
        if ($this->getUser()?->hasPermission(PermissionEnum::VIEW_THEME)) {
            $actions[] = [
                'name' => 'details',
                'label' => $this->translator->trans('indium.crud.theme.show_details'),
                'icon' => 'fa fa-eye',
            ...
            foreach ($theme->getContexts() as $context) {
                if (!$theme->isActiveInContext($context)) {
                    $contextLabel = match($context) {
                        'panel' => $this->translator->trans('indium.crud.theme.context_panel'),
                        'landing' => $this->translator->trans('indium.crud.theme.context_landing'),
                        'email' => $this->translator->trans('indium.crud.theme.context_email'),
                    ...
                    $actions[] = [
                        ...
                        'label' => sprintf(
                            $this->translator->trans('indium.crud.theme.set_as_default_in'),
                            $contextLabel
                        ),
                        'icon' => 'fa fa-check',
                        'url' => '#',
        ...
        if ($this->getUser()?->hasPermission(PermissionEnum::COPY_THEME)) {
            $actions[] = [
                'name' => 'copy',
                'label' => $this->translator->trans('indium.crud.theme.copy_theme'),
                'icon' => 'fa fa-copy',
                'url' => '#',
                'class' => 'primary',
                'data_attrs' => [
        ...
        if ($this->getUser()?->hasPermission(PermissionEnum::EXPORT_THEME)) {
            $actions[] = [
                'name' => 'export',
                'label' => $this->translator->trans('indium.crud.theme.export_theme'),
                'icon' => 'fa fa-download',
                'url' => $this->adminUrlGenerator
                    ->setController(self::class)
        ...
        if (!$theme->isActiveInAnyContext()
            ...
            $actions[] = [
                'name' => 'delete',
                'label' => $this->translator->trans('indium.crud.theme.delete_theme'),
                'icon' => 'fa fa-trash',
                'url' => '#',
                'class' => 'danger',
                'data_attrs' => [
    ...
    private function checkFilesystemPermissions(string $redirectUrl): ?RedirectResponse
    {
        ...
        if (!empty($unwritable)) {
            $this->addFlash('danger', $this->translator->trans(
                'indium.theme.upload.filesystem_permission_error',
