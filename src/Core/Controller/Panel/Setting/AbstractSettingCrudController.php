...
abstract class AbstractSettingCrudController extends AbstractPanelController
{
    ...
    {
        $fields = [
            TextField::new('name', $this->translator->trans('indium.crud.setting.name'))
                ->setDisabled($pageName === Crud::PAGE_EDIT)
                ->formatValue(function ($value) {
                    $hintIndex = "indium.crud.setting.hints.$value";
                ...
                ->setColumns(6),
            ChoiceField::new('type', $this->translator->trans('indium.crud.setting.type'))
        ...
        $valueLabel = $this->translator->trans('indium.crud.setting.value');
        ...
        if ($pageName === Crud::PAGE_INDEX) {
                ...
                ->formatValue(function ($value, $entity) {
                    ...
                    return match ($entity->getType()) {
                        ...
                        SettingTypeEnum::BOOLEAN->value => $value
                            ? $this->translator->trans('indium.crud.setting.yes')
                            : $this->translator->trans('indium.crud.setting.no'),
            ...
            $valueField = match ($normalizedType) {
                ...
                SettingTypeEnum::BOOLEAN->value => ChoiceField::new('value', $valueLabel)
                    ->setChoices([
                        $this->translator->trans('indium.crud.setting.yes') => '1',
                        $this->translator->trans('indium.crud.setting.no') => '0',
                    ])
        ...
        if ($isNullable) {
            ...
            $fields[] = BooleanField::new('setAsEmpty', $this->translator->trans('indium.crud.setting.set_as_empty'))
                ->setHelp($this->translator->trans('indium.crud.setting.set_as_empty_help'))
                ->setColumns(6)
        ...
        $fields[] = ChoiceField::new('context', $this->translator->trans('indium.crud.setting.context'))
                ->setChoices(SettingContextEnum::getValues())
                ->setRequired(true)
                ->setColumns(6)
                ->hideOnIndex()
                ->hideOnForm();
        $fields[] = NumberField::new('hierarchy', $this->translator->trans('indium.crud.setting.hierarchy'))
                ->setRequired(true)
                ->setColumns(6)
                ->hideOnIndex()
                ->hideOnForm();
    ...
    {
        $actions = $actions
            ->update(Crud::PAGE_INDEX, Action::NEW, fn (Action $action) => $action->setLabel($this->translator->trans('indium.crud.setting.add')))
            ->update(Crud::PAGE_NEW, Action::SAVE_AND_RETURN, fn (Action $action) => $action->setLabel($this->translator->trans('indium.crud.setting.add')))
            ->update(Crud::PAGE_EDIT, Action::SAVE_AND_RETURN, fn (Action $action) => $action->setLabel($this->translator->trans('indium.crud.setting.save')))
    ...
    public function configureCrud(Crud $crud): Crud
    {
        ...
        $crud
            ->setEntityLabelInSingular(sprintf('%s %s', $contextLabel, $this->translator->trans('indium.crud.setting.setting')))
            ->setEntityLabelInPlural(sprintf('%s %s', $contextLabel, $this->translator->trans('indium.crud.setting.settings')))
    ...
    public function persistEntity(EntityManagerInterface $entityManager, $entityInstance): void
    {
        try {
            ...
            $this->addFlash('success', $this->translator->trans('indium.crud.setting.created_successfully'));
        } catch (Exception $e) {
            $this->addFlash('danger', $this->translator->trans('indium.crud.setting.create_error', ['%error%' => $e->getMessage()]));
            throw $e;
        }
    }
    ...
    public function updateEntity(EntityManagerInterface $entityManager, $entityInstance): void
    {
        try {
            ...
            $this->addFlash('success', $this->translator->trans('indium.crud.setting.updated_successfully'));
        } catch (Exception $e) {
            $this->addFlash('danger', $this->translator->trans('indium.crud.setting.update_error', ['%error%' => $e->getMessage()]));
            throw $e;
        }
    }
    ...
    private function getHelpText(?string $name): string
    {
        ...
        $hintIndex = "indium.crud.setting.hints.$name";
        $hint = $this->translator->trans($hintIndex);
