...
class PterodactylSettingCrudController extends AbstractSettingCrudController
{
    ...
    {
        $testPterodactylAction = Action::new('testPterodactylConnection', $this->translator->trans('indium.crud.setting.test_pterodactyl_connection'))
    ...
    public function testPterodactylConnection(): RedirectResponse
    {
        try {
            ...
            if ($result->isVerificationSuccessful) {
                $this->addFlash('success', $this->translator->trans('indium.crud.setting.pterodactyl_connection_success'));
            } else {
                $this->addFlash('danger', $this->translator->trans('indium.crud.setting.pterodactyl_connection_failed'));
            }
