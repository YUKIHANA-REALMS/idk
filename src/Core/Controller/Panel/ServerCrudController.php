...
class ServerCrudController extends AbstractPanelController
{
    ...
    public function configureFields(string $pageName): iterable
    {
        ...
        $this->fields = [
            IdField::new('id')
                ->hideOnForm(),
            IntegerField::new('pterodactylServerId', $this->translator->trans('indium.crud.server.pterodactyl_server_id'))
                ->setDisabled()
                ->onlyOnForms()
                ->setColumns(4),
            TextField::new('pterodactylServerIdentifier', $this->translator->trans(
                $pageName === Crud::PAGE_INDEX
                    ? 'indium.crud.server.pterodactyl_server_identifier_short'
                    : 'indium.crud.server.pterodactyl_server_identifier'
            ))
                ->setDisabled()
                ->setColumns(4),
            TextField::new('name', $this->translator->trans('indium.crud.server.name'))
                ->formatValue(function ($value, Server $entity) {
                    return $value ?: ($entity->getServerProduct()?->getName() ?? 'N/A');
                }),
            AssociationField::new('serverProduct', $this->translator->trans('indium.crud.server.product_server_build'))
                ->setDisabled()
                ->setColumns(4),
            AssociationField::new('user', $this->translator->trans('indium.crud.server.user'))
                ->setQueryBuilder(function ($queryBuilder) {
                    return $queryBuilder
                        ->andWhere('entity.deletedAt IS NULL');
                })
                ->setColumns(4),
            DateTimeField::new('expiresAt', $this->translator->trans('indium.crud.server.expires_at')),
            BooleanField::new('autoRenewal', $this->translator->trans('indium.crud.server.auto_renewal'))
                ->hideOnIndex()
                ->setColumns(4),
            NumberField::new('serverProduct.diskSpace', sprintf('%s (MB)', $this->translator->trans('indium.crud.product.disk_space')))
                ->onlyOnDetail()
                ->formatValue(fn($value) => $value ?? 'N/A'),
            NumberField::new('serverProduct.memory', sprintf('%s (MB)', $this->translator->trans('indium.crud.product.memory')))
                ->onlyOnIndex()
                ->formatValue(fn($value) => $value ?? 'N/A'),
            NumberField::new('serverProduct.cpu', sprintf('%s (%%)', $this->translator->trans('indium.crud.product.cpu')))
                ->onlyOnIndex()
                ->formatValue(fn($value) => $value ?? 'N/A'),
            BooleanField::new('isSuspended', $this->translator->trans('indium.crud.server.is_suspended'))
                ->setColumns(4),
            Field::new('healthStatus', $this->translator->trans('indium.crud.server.health_status'))
                ->onlyOnIndex()
                ->setColumns(2)
                ->setSortable(false)
            ...
            DateTimeField::new('createdAt', $this->translator->trans('indium.crud.server.created_at'))
                ->onlyOnDetail(),
            DateTimeField::new('deletedAt', $this->translator->trans('indium.crud.server.deleted_at'))
                ->onlyOnDetail(),
        ];
    ...
    public function configureActions(Actions $actions): Actions
    {
        $actions = $actions
            ...
            ->update(
                ...
                fn (Action $action) => $action->setLabel($this->translator->trans('indium.crud.server.save')),
    ...
    public function configureCrud(Crud $crud): Crud
    {
        ...
        $crud
            ->setEntityLabelInSingular($this->translator->trans('indium.crud.server.server'))
            ->setEntityLabelInPlural($this->translator->trans('indium.crud.server.servers'))
    ...
    public function deleteEntity(EntityManagerInterface $entityManager, $entityInstance): void
    {
        try {
            ...
            $this->addFlash('success', $this->translator->trans('indium.crud.server.deleted_successfully'));
        } catch (Exception $e) {
            $this->addFlash('danger', $this->translator->trans('indium.crud.server.delete_error', ['%error%' => $e->getMessage()]));
        }
    }
    ...
    private function getServerProductAction(string $action): Action
    {
        ...
        return Action::new(
            ...
            $this->translator->trans(sprintf('indium.crud.server.server_product_%s', $action)),
    ...
    {
        return Action::new('createServer', $this->translator->trans('indium.crud.server.create_server'))
            ->setIcon('fa fa-plus')
            ->linkToUrl(
                fn () => $this->generateUrl(
                    'panel',
