...
class RegistrationController extends AbstractController
{
    ...
    public function register(
        ...
        if ($form->isSubmitted() && $form->isValid()) {
            ...
            if ($submitEvent->isPropagationStopped()) {
                $registrationErrors[] = $this->translator->trans('indium.register.plugin_validation_failed');
            } else {
    ...
    public function verifyEmail(
        ...
        if (!$token) {
            $this->addFlash('danger', $this->translator->trans('indium.register.verification_token_invalid'));
            return $this->redirectToRoute('panel');
        }
        try {
            $this->registrationService->verifyEmail($token);
            $this->addFlash('success', $this->translator->trans('indium.register.verification_success'));
        } catch (Exception $e) {
    ...
    public function resendVerification(): Response
    {
        ...
        if (!$user instanceof UserInterface) {
            $this->addFlash('danger', $this->translator->trans('indium.email.verification.not_logged_in'));
            return $this->redirectToRoute('app_login');
        }

        if ($user->isVerified()) {
            $this->addFlash('info', $this->translator->trans('indium.email.verification.already_verified'));
            return $this->redirectToRoute('panel');
        }

        try {
            $this->emailVerificationService->resendVerificationEmail($user);
            $this->addFlash('success', $this->translator->trans('indium.email.verification.resend_success'));
        } catch (Exception $e) {
            $this->addFlash('danger', $e->getMessage());
        }
