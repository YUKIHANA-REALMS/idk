...
class ServerScheduleController extends APIAbstractController
{
    ...
    public function createSchedule(int $id, Request $request): JsonResponse
    {
        ...
        if (empty($data['name'])) {
            ...
            $response->setData(['error' => $this->translator->trans('indium.api.schedule.name_required')]);
            return $response;
        }
        ...
        foreach ($cronFields as $field) {
            if (!isset($data[$field]) || $data[$field] === '') {
                $response->setStatusCode(400);
                $response->setData(['error' => $this->translator->trans('indium.api.schedule.cron_field_required', ['field' => $field])]);
                return $response;
            }
        }
    ...
    private function validateScheduleTaskData(?array $data): ?JsonResponse
    {
        if (empty($data['action'])) {
            $response = new JsonResponse();
            $response->setStatusCode(400);
            $response->setData(['error' => $this->translator->trans('indium.api.schedule.action_required')]);
            return $response;
        }
        ...
        if (empty($data['payload'])) {
            $response = new JsonResponse();
            $response->setStatusCode(400);
            $response->setData(['error' => $this->translator->trans('indium.api.schedule.payload_required')]);
            return $response;
        }
