...
use Indium\PterodactylAddon\Http\Requests\SSOLoginRequest;
use Indium\PterodactylAddon\Services\JwtService;
...
class SSOAuthorizationController extends AbstractLoginController
{
    ...
    {
        $token = $request->post('indium_sso_token');
        ...
        return redirect()->intended($request->post('indium_sso_redirect') ?? '/');
    }
}
